...
- 用户指出：内容上不说人话、各处解释不够循序渐进、有知识的诅咒、巩固知识的小题表述不清。
- 已系统性改写全部 11 个 HTML 文件（4 个线性页 + 4 个题型页 + 首页 + 样章 + 考前作战页）的 QA 答案：
  - `标准分析（为什么）` → `为什么这是对的：`：原来是一个通用模板"这道小题问的是...标准答案里的...只是结论，真正要拿分的是说明题面为什么支持这个结论。" + 课程级兜底句。现在每条都写成了针对该题的具体推理，带例子和分步逻辑。
  - `题目语境怎么用` 改为该题所属 section 的具体考试场景。
  - `卷面写法` 改为该题类型的可交卷表达。
  - `常见失分点` 改为该题特有的易错点。
  - 删除所有元叙述："这道小题问的是"、"标准分析（为什么）" 等模板文本已全部清除。
- 验证结果：
  - 全站 `.html` 文件中 `标准分析（为什么）` 残留为 0。
  - 全站 `.html` 文件中 `这道小题问的是` 残留为 0。
  - `为什么这是对的：` 共 803 处，覆盖全部 720 张 QA 卡片。
  - HTML 结构、CSS class、`qa-hardcoded` 标记、section ID、锚点全部保持不变。
  - 清理了 agent 留下的 `.test` 和 `.bak` 临时文件。

### 2026-05-13（续）

- 用户进一步指出：小题题目本身表述不清，例如"考试5. Cs615 考试补题：2025 Q1(a) fullstack 题如何举例?"没有说清楚原题到底问了什么。
- 已从本地 11 份真题 PDF 中提取了全部题目的原始问法（包括 2023/2024/2025 真题和 Sample A/B）。
- 已系统性改写全部 11 个 HTML 文件中所有"考试补题"和"首页考试补题"的问题文字：
  - 每个问题现在都包含：课程代码 + 年份 + 学期 + 题号(含子题字母) + 原题场景关键词
  - 例如 "2025 Q1(a) fullstack 题如何举例" → "CS615 2025 Summer Q1(a)：給培訓實習生解釋 full-stack 各層角色時，用什麼具體例子最有說服力？"
  - 例如 "Sample FinTrack recommendation 怎样不空泛" → "CS615 Sample Q1(c) FinTrack 案例：推薦 Approach A 或 B 時，需要從哪三個維度論證才不空泛？"
  - 例如 "2025 decideWrite line 33 red 要怎么补" → "CS608 2025 Summer Q2(a)：decideWrite() JaCoCo 報告 line 33 紅色(statement 未執行)，要設計什麼樣的輸入才能走到這行？"
- 验证：全站不再有缺少子题字母的模糊题号引用。
