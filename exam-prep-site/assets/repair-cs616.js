(function () {
  ...
  function progressStates(title, headers, rows, operations) {
    return rows.map((row, current) => ({
      after: current + 1,
      title: title + "：完成第 " + (current + 1) + " 行",
      operation: operations[current],
      caption: "表格始终保留全部行。未完成的行显示“待计算”。箭头标出本步刚写入的行。",
      headers: headers.concat("状态"),
      rows: rows.map((item, index) => {
        if (index > current) {
          return [item[0]].concat(headers.slice(1).map(() => pending), "未计算");
        }
        return item.concat(index === current ? "← 当前行" : "已完成");
      })
    }));
  }
  ...
  if (classicalAndZk) {
    ...
    const affineRows = [
      ["约定", "字母与模数", "A=0,…,Z=25；N=26²", "N=676"],
      ["编码 ma", "首字母 m=12，次字母 a=0", "26×12+0", "312"],
      ["编码 il", "首字母 i=8，次字母 l=11", "26×8+11", "219"],
      ["编码 uw", "首字母 u=20，次字母 w=22", "26×20+22", "542"],
      ["编码 ex", "首字母 e=4，次字母 x=23", "26×4+23", "127"],
      ["列方程", "ma→uw；il→ex", "312a+b≡542；219a+b≡127 (mod 676)", "两条同余式"],
      ["相减消 b", "第一式减第二式", "(312−219)a≡542−127", "93a≡415 (mod 676)"],
      ["检查逆元", "题给 93⁻¹=189", "93×189=17577=26×676+1", "逆元正确"],
      ["求 a", "两边乘 189", "a≡415×189=78435≡19 (mod 676)", "a=19"],
      ["回代求 b", "代回 312a+b≡542", "b≡542−19×312=−5386≡22 (mod 676)", "b=22"],
      ["检查 ma", "m=312", "19×312+22=5950≡542 (mod 676)", "542 解码为 uw"],
      ["检查 il", "m=219", "19×219+22=4183≡127 (mod 676)", "127 解码为 ex"]
    ];
    const affineOperations = [
      "写入本题的字母编号和模数。以后每次取模都使用 676。",
      "查出两个字母的编号。这里字母 a 的编号是 0，不是密钥参数 a。把首字母编号乘 26，再加次字母编号。",
      "查出 i 和 l 的编号。继续使用“首字母×26+次字母”的公式。",
      "把密文对 uw 编成一个 0 到 675 的数。",
      "把密文对 ex 编成一个 0 到 675 的数。",
      "把两组明文编号和密文编号分别代入 c≡am+b。",
      "用第一条同余式减第二条同余式。两边的 b 相减后都是 0。",
      "在使用 189 前，先乘回 93。余数为 1，所以 189 是逆元。",
      "同余式两边乘 189。再把乘积除以 676，并保留余数 19。",
      "把 a=19 代回第一条原式。把负结果加 8×676，得到标准余数 22。",
      "用 a=19、b=22 重新加密 ma。结果必须等于 uw 的编号 542。",
      "用相同密钥重新加密 il。结果必须等于 ex 的编号 127。"
    ];
    classicalAndZk.example = {
      ...
      states: progressStates("mail→uwex 完整计算表", ["阶段", "本行输入", "本行计算", "本行结果"], affineRows, affineOperations),
      result: "Affine digraph 密钥是 a=19，b=22。",
      check: "19×312+22≡542，19×219+22≡127 (mod 676)。两组原始映射都成立。"
    };

    const smallZkRows = [
      ["公开数据", "p=11，g=2，y=3", "公开陈述：知道 x，使 2ˣ≡3 (mod 11)", "攻击者不使用 x"],
      ["验证规则", "commitment=t，challenge=c，response=R", "2ᴿ≡t×yᶜ (mod 11)", "本题 c 固定为 1"],
      ["先选 response", "R=4", "攻击者在发送 t 前已经知道 c=1", "R 已固定"],
      ["计算左边", "2ᴿ", "2⁴=16=1×11+5", "2ᴿ≡5"],
      ["Euclid 1", "11 和 3", "11=3×3+2", "余数 2"],
      ["Euclid 2", "3 和上一行余数 2", "3=1×2+1", "余数 1，逆元存在"],
      ["反代 1", "3=1×2+1", "把 1×2 移到等号右边：1=3−1×2", "1=3−2"],
      ["反代 2", "11=3×3+2", "先单独解出 2：2=11−3×3", "2=11−3×3"],
      ["反代 3", "1=3−2", "把 2 替换为 11−3×3：1=3−(11−3×3)", "只替换 2，其余不动"],
      ["合并同类项", "1=3−(11−3×3)", "去括号：1=3−11+3×3=4×3−11", "3 的系数是 4"],
      ["读出逆元", "1=4×3−1×11", "等号右边对 11 取模，−1×11 的余数为 0", "3⁻¹≡4 (mod 11)"],
      ["反算 commitment", "t≡2ᴿ×3⁻¹", "t≡5×4=20≡9 (mod 11)", "t=9"],
      ["发送 transcript", "先发 t=9；收到 c=1；再发 R=4", "(t,c,R)=(9,1,4)", "消息齐全"],
      ["验证", "左边 2⁴；右边 9×3¹", "2⁴≡5；9×3=27≡5 (mod 11)", "等式成立"],
      ["结论", "计算未使用 witness x", "攻击者只为已知的 c=1 配好 t 和 R", "Soundness 失败"]
    ];
    const smallZkOperations = [
      "写公开数据，并区分公开陈述和 witness。攻击计算不需要知道 witness。",
      "先写 verifier 实际检查的等式。固定 c=1 后，右边变成 t×3。",
      "攻击者先选一个方便的 response R=4。这一步发生在发送 commitment 之前。",
      "计算验证等式左边。16 除以 11 的余数是 5。",
      "第一次 Euclid 除法：11 除以 3，商是 3，余数是 2。只记录这一行，不反代。",
      "第二次 Euclid 除法：3 除以 2，商是 1，余数是 1。出现余数 1，说明 gcd(3,11)=1。",
      "从最后一条除法等式开始。3=1×2+1，把乘积 1×2 移到右边，得到 1=3−2。",
      "回到上一条除法等式。11=3×3+2，把 3×3 移到右边，单独写成 2=11−3×3。",
      "上一行要消去的是 2，所以把 1=3−2 中的 2 整块替换为 11−3×3。不要改动前面的 3。",
      "去掉括号时，括号前有负号：−(11−3×3)=−11+3×3。再把 3+3×3 合并为 4×3。",
      "式子 1=4×3−11 表示 4×3 除以 11 的余数是 1。因此 4 是 3 的模 11 逆元。",
      "把验证等式改写为 t≡2ᴿ×3⁻¹，再代入 5 和 4。",
      "按协议表面顺序发送三条消息。数值已经在发送 commitment 前配好。",
      "分别计算验证等式两边。两边的余数都是 5。",
      "记录缺少的证明内容。攻击者没有展示自己能回答另一个 challenge。"
    ];
    const smallZk = {
      ...
      states: progressStates("固定 challenge 伪造表", ["阶段", "本行输入", "本行计算", "本行结果"], smallZkRows, smallZkOperations),
      result: "伪造 transcript 是 (commitment=9, challenge=1, response=4)。",
      check: "2⁴ mod 11=5。9×3 mod 11=5。验证通过，但计算没有使用 witness。"
    };

    const realZkRows = [
      ["验证规则", "p=991，g=2，y=697，c=1", "2ᴿ≡t×697 (mod 991)", "要求构造 t 和 R"],
      ["先选 response", "R=333", "333=5×64+13", "先算 2³³³"],
      ["算 2¹³", "8192", "8192−8×991", "264"],
      ["算 827²", "题给 2⁶⁴≡827", "827²=683929≡139 (mod 991)", "139"],
      ["算 827⁴", "139²", "19321−19×991", "492"],
      ["算 827⁵", "492×827", "406884−410×991", "574"],
      ["合并幂", "2³³³=(2⁶⁴)⁵×2¹³", "574×264=151536−152×991", "2³³³≡904"],
      ["Euclid 1", "991 和 697", "991=1×697+294", "r=294"],
      ["Euclid 2", "697 和 294", "697=2×294+109", "r=109"],
      ["Euclid 3", "294 和 109", "294=2×109+76", "r=76"],
      ["Euclid 4", "109 和 76", "109=1×76+33", "r=33"],
      ["Euclid 5", "76 和 33", "76=2×33+10", "r=10"],
      ["Euclid 6", "33 和 10", "33=3×10+3", "r=3"],
      ["Euclid 7", "10 和 3", "10=3×3+1", "r=1"],
      ["反代 1", "10=3×3+1", "1=10−3×3", "消去余数 1 的来源等式"],
      ["反代 2", "33=3×10+3，所以 3=33−3×10", "1=10−3×(33−3×10)=10×10−3×33", "只把 3 替换掉"],
      ["反代 3", "76=2×33+10，所以 10=76−2×33", "1=10×(76−2×33)−3×33=10×76−23×33", "只把 10 替换掉"],
      ["反代 4", "109=1×76+33，所以 33=109−76", "1=10×76−23×(109−76)=33×76−23×109", "只把 33 替换掉"],
      ["反代 5", "294=2×109+76，所以 76=294−2×109", "1=33×(294−2×109)−23×109=33×294−89×109", "只把 76 替换掉"],
      ["反代 6", "697=2×294+109，所以 109=697−2×294", "1=33×294−89×(697−2×294)=211×294−89×697", "只把 109 替换掉"],
      ["反代 7", "991=1×697+294，所以 294=991−697", "1=211×(991−697)−89×697=211×991−300×697", "只把 294 替换掉"],
      ["读出逆元", "1=211×991−300×697", "对 991 取模：1≡−300×697；−300+991=691", "697⁻¹≡691 (mod 991)"],
      ["检查逆元", "697×691", "481627=486×991+1", "余数为 1"],
      ["反算 commitment", "t≡904×691", "624664=630×991+334", "t=334"],
      ["发送 transcript", "t=334，c=1，R=333", "(334,1,333)", "消息齐全"],
      ["验证", "t×697", "334×697=232798=234×991+904", "右边余数为 904"],
      ["比较", "左边 2³³³≡904", "右边 334×697≡904", "验证通过"]
    ];
    const realZkOperations = [
      "把公开数字代入验证等式。c=1 已经固定，所以攻击者在 commitment 前知道完整右边形式。",
      "先选 response R=333。使用题给 2⁶⁴ 的余数来计算大幂。",
      "计算 2¹³，并减去 8 个 991。",
      "平方 827。减去 689 个 991 后仍大于 991，再减一个 991，得到 139。",
      "平方上一行的 139。除以 991 后保留余数 492。",
      "用 827⁵=827⁴×827。把 406884 减去 410×991，得到 574。",
      "把 827⁵ 的余数和 2¹³ 的余数相乘。最终得到 2³³³≡904。",
      "开始 Euclid 除法。用 991 除以 697，并记录余数 294。",
      "把上一行的除数和余数移到本行。计算新余数 109。",
      "继续相同的 Euclid 更新。记录余数 76。",
      "继续除法。记录余数 33。",
      "继续除法。记录余数 10。",
      "继续除法。记录余数 3。",
      "最后得到余数 1，所以逆元存在。",
      "从最后一条等式 10=3×3+1 开始，把乘积移到右边，单独留下 1=10−3×3。",
      "上一条等式给出 3=33−3×10。把它代入上一行的 3；展开后，10 的系数由 1+9 变成 10。",
      "再用 10=76−2×33 替换全部的 10。展开：10×76−20×33−3×33=10×76−23×33。",
      "再用 33=109−76 替换全部的 33。括号前是 −23，所以展开后得到 −23×109+23×76；与原来的 10×76 合并为 33×76。",
      "再用 76=294−2×109 替换全部的 76。展开后，109 的系数是 −66−23=−89。",
      "再用 109=697−2×294 替换全部的 109。展开后，294 的系数是 33+178=211。",
      "最后用 294=991−697 替换全部的 294。展开后，697 的系数是 −211−89=−300。",
      "现在 1=211×991−300×697。模 991 下，211×991 消失，所以 −300 是逆元；加 991 得到标准正余数 691。",
      "把 691 乘回 697。余数是 1，所以逆元正确。",
      "把验证等式两边乘 697⁻¹。计算 t≡904×691≡334。",
      "先发送 commitment 334。固定 challenge 到达后，发送 response 333。",
      "计算 verifier 看到的右边。结果的余数是 904。",
      "比较验证等式两边。两边都是 904，所以这条 transcript 通过。"
    ];
    const realZk = {
      ...
      states: progressStates("2026 固定 challenge 完整计算表", ["阶段", "本行输入", "本行计算", "本行结果"], realZkRows, realZkOperations),
  ...
  if (cs616Overview && cs616Overview.learn && cs616Overview.learn[1]) {
    Object.assign(cs616Overview.learn[1], {
      ...
      c: "独立完成 mail→uwex 的全部编码、消元、求逆、回代和检查。再完成 mod 11 与 2026 数字的固定 challenge 伪造表。"
    });
  }
