(function () {
  ...
  function garnerRows(filled, current) {
    const data = [
      ["(415,62)", "62−415=−353", "−353 mod 547=194", "194×433", "84002 mod 547=311", "415+523×311=163068"],
      ["(415,485)", "485−415=70", "70", "70×433", "30310 mod 547=225", "415+523×225=118090"],
      ["(108,62)", "62−108=−46", "−46 mod 547=501", "501×433", "216933 mod 547=321", "108+523×321=167991"],
      ["(108,485)", "485−108=377", "377", "377×433", "163241 mod 547=235", "108+523×235=123013"]
    ];
    return data.map((row, index) => {
      if (index >= filled) {
        return [row[0], pending, pending, pending, pending, pending, "未计算"];
      }
  ...
  if (rootsPart) {
    ...
    const headers = ["(a_p,a_q)", "差值 a_q−a_p", "负数归一到 mod 547", "乘逆元 433", "直接计算 u=(乘积) mod 547", "x=a_p+523u", "状态"];
  ...
  if (smallRsa) {
    smallRsa.steps = [
      ...
      "填 Step 1。q=⌊40÷3⌋=13。r=40−13×3=1。d=0−13×1=−13。同行的 r=1，所以 3⁻¹≡−13≡27 (mod 40)。RSA 把私钥指数定义为 d=e⁻¹ mod φ(n)；这里 e=3、φ(n)=40，因此 d=27。检查 3×27=81≡1 (mod 40)。",
      "回到 RSA 签名公式 s≡mᵈ (mod n)。代入消息 m=9、刚求出的 d=27 和 n=55，得到 s≡9²⁷ (mod 55)。先把基数行写成 9¹ mod 55=9，其余平方行先保留为待计算。",
      "平方 9¹，然后直接取模：9² mod 55=81 mod 55=26。",
      "平方上一行余数 26，然后直接取模：26² mod 55=676 mod 55=16，所以 9⁴ mod 55=16。",
      "平方上一行余数 16，然后直接取模：16² mod 55=256 mod 55=36，所以 9⁸ mod 55=36。",
      "平方上一行余数 36，然后直接取模：36² mod 55=1296 mod 55=31，所以 9¹⁶ mod 55=31。",
      "把指数拆成 27=16+8+2+1。选择表中的 31、36、26 和 9。逐次乘模表先保留三行。",
      "第一次乘模：31×36 mod 55=1116 mod 55=16，所以累计余数是 16。",
      "第二次乘模：16×26 mod 55=416 mod 55=31，所以累计余数是 31。",
      "第三次乘模：31×9 mod 55=279 mod 55=4，所以签名 s=4。",
    ...
    const multiplyData = [
      ["1", "31", "36", "31×36=1116", "1116 mod 55=16"],
      ["2", "16", "26", "16×26=416", "416 mod 55=31"],
      ["3", "31", "9", "31×9=279", "279 mod 55=4"]
    ];
    function multiplyRows(filled, current) {
      return multiplyData.map((row, index) => index < filled
        ? row.concat(index === current ? "← 当前行" : "已完成")
        : [row[0], pending, pending, pending, pending, "未计算"]);
    }
    smallRsa.states = [
      {
        after: 1,
      ...
      {after: 9, title: "逐次乘模固定工作表：选择 16、8、2、1 次幂", headers: ["轮", "累计值", "乘入余数", "乘法", "直接取模后的新余数", "状态"], rows: multiplyRows(0, -1)},
      {after: 10, title: "逐次乘模固定工作表：完成第 1 轮", headers: ["轮", "累计值", "乘入余数", "乘法", "直接取模后的新余数", "状态"], rows: multiplyRows(1, 0)},
      {after: 11, title: "逐次乘模固定工作表：完成第 2 轮", headers: ["轮", "累计值", "乘入余数", "乘法", "直接取模后的新余数", "状态"], rows: multiplyRows(2, 1)},
      {after: 12, title: "逐次乘模固定工作表：完成第 3 轮", headers: ["轮", "累计值", "乘入余数", "乘法", "直接取模后的新余数", "状态"], rows: multiplyRows(3, 2)},
      {
        after: 13,
        title: "签名回代检查",
  ...
  if (eccExample) {
    eccExample.steps = [
      ...
      "回到倍点斜率公式。分子已经约简为 9，分母是 2，所以先写 λ≡9×2⁻¹ (mod 17)。上一步已经得到 2⁻¹≡9 (mod 17)；把 2⁻¹ 替换为 9，得到 λ≡9×9 mod 17=13。",
    ...
    const doublingData = [
      ...
      ["分母", "2×1", "2 mod 17", "2"],
      ["分母逆元", "2⁻¹≡−8≡9 (mod 17)", "−8", "9"],
      ["斜率 λ", "λ≡9×2⁻¹ (mod 17)", "代入 2⁻¹≡9：9×9 mod 17", "13"],
  ...
  if (classicalAndZk) {
    ...
    const affineOperations = [
      ...
      "题目给 93⁻¹≡189 (mod 676)。这句话的定义是：93×189 除以 676 的余数为 1，也就是 93×189≡1 (mod 676)。",
      "把给定逆元乘回去检查。93×189=17577，而 17577=26×676+1。因此余数确实是 1，题目给的 189 可以使用。",
      "回到 415≡93a (mod 676)。我们的目标是消掉 a 前面的 93，所以在等式左右两边都乘 189：189×415≡189×93a (mod 676)。",
      "只整理右边的乘法顺序。189×93a=(93×189)a，所以右边可以写成 (93×189)a。",
      "前面已经验证 93×189≡1 (mod 676)，因此 (93×189)a≡1a≡a (mod 676)。原式现在变成 189×415≡a (mod 676)。",
      "计算左边：189×415=78435。再除以 676：78435=116×676+19，所以 189×415≡19 (mod 676)。",
      "把两条结论合在一起：189×415≡a (mod 676)，同时 189×415≡19 (mod 676)，因此 a≡19 (mod 676)。取 0 到 675 之间的标准代表元，得到 a=19。",
    ...
    const realZkOperations = [
      ...
      "拆指数：333=5×64+13，所以 2³³³=(2⁶⁴)⁵×2¹³。题目给 2⁶⁴≡827 (mod 991)；另外 2¹³=8192=8×991+264，所以 2¹³≡264 (mod 991)。",
      "连续平方只写需要的量：827²=683929=690×991+139，所以 827²≡139 (mod 991)；827⁴≡139²≡492 (mod 991)，其中 139²=19321=19×991+492。",
    ...
    const realZk = {
      ...
      prompt: "verifier 固定发送 c=1，并检查 2ᴿ≡commitment×697 (mod 991)。使用 R=333 构造 commitment。",
      given: "题给 2⁶⁴ mod 991=827。需要计算 2³³³、697⁻¹、commitment 和最终验证。",
      target: "写出完整 transcript，并证明验证等式两边都等于 904。",
      steps: realZkOperations,
      states: eeaSequence(inv697, 6),
      result: "伪造 transcript 是 (commitment=334, challenge=1, response=333)。",
      check: "2³³³≡904 (mod 991)。697⁻¹≡691 (mod 991)。904×691≡334 (mod 991)。334×697≡904 (mod 991)。"
    };
  ...
  putState(rootsPart, {
    after: 18,
    title: "四个平方根逐一平方回代",
    caption: "本题允许直接计算 mod，因此不再额外求商。每行把候选根直接代入 x² mod 286081，并把余数与目标 229755 比较。",
    headers: ["候选根 x", "直接输入计算器的式子", "余数", "与目标 229755 比较"],
    rows: [
      ["163068", "163068² mod 286081", "229755", "相等，通过"],
      ["118090", "118090² mod 286081", "229755", "相等，通过"],
      ["167991", "167991² mod 286081", "229755", "相等，通过"],
      ["123013", "123013² mod 286081", "229755", "相等，通过"]
    ]
  });
  putState(rootsPart, {
    after: 19,
    ...
    rows: [[
      "y²≡x³+13x+b (mod 37)",
      "14²≡25³+13×25+b (mod 37)",
      "b≡14²−25³−13×25 (mod 37)",
      "b≡−15754 (mod 37)"
    ]]
  });
  putState(rootsPart, {
    ...
    rows: [
      ...
      ["分母", "2y", "2×14=28", "28", "需要求 28⁻¹ mod 37"],
      ["λ", "分子×分母⁻¹ (mod 37)", pending, pending, "待计算"]
    ]
  });
  ...
  putState(rootsPart, {
    after: 23,
    title: "把分母逆元代回 2P 的斜率公式",
    headers: ["当前斜率式", "已求出的逆元", "执行替换", "直接取模", "结果"],
    rows: [[
      "λ≡1×28⁻¹ (mod 37)",
      "28⁻¹≡4 (mod 37)",
      "λ≡1×4 (mod 37)",
  ...
  rootsPart.states = (rootsPart.states || []).filter((state) => state.after < 24 || state.after > 29);
  ...
  if (rabinPart) {
    rabinPart.steps[1] = "攻击者选 R=23769451，直接计算 C=R² mod N=23769451² mod 47479253=23004433。";
    rabinPart.steps[2] = "Oracle 返回 Y=31423469。不能只接受这个数字；直接计算 Y² mod N=31423469² mod 47479253=23004433，与 C 相同。再检查 Y 不是 R 或 −R 的同余类。";
    rabinPart.steps[23] = "验证两个 gcd 的乘积：13523×3511=13523×3500+13523×11=47330500+148753=47479253=N。";
  }
  putState(rabinPart, {
    after: 2,
    title: "完整计算 C=R² mod N",
    headers: ["R", "直接输入计算器的式子", "C"],
    rows: [["23769451", "23769451² mod 47479253", "23004433"]]
  });
  putState(rabinPart, {
    after: 3,
    title: "检查 oracle 返回值确实是同一密文的平方根",
    headers: ["Y", "直接输入计算器的式子", "余数", "与 C 比较"],
    rows: [["31423469", "31423469² mod 47479253", "23004433", "等于 C"]]
  });
  putState(rabinPart, {
    after: 24,
    title: "两个 gcd 重新乘回 N",
  ...
  putState(ecdsaPart, {
    ...
    headers: ["H(M)", "q", "直接取模", "h"],
    rows: [["22", "7", "22 mod 7", "1"]]
  });
  putState(ecdsaPart, {
    after: 3,
    title: "计算 u₁",
  ...
  putState(ecdsaPart, {
    ...
    rows: [["6P=(13,15)", "6Y=(1,14)", "14−15≡−1 (mod 17)", "1−13≡−12 (mod 17)", "λ≡(−1)×(−12)⁻¹ (mod 17)"]]
  });
  putState(ecdsaPart, {
    after: 9,
