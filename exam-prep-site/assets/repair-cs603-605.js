(() => {
  ...
  const binaryCflConfig = {
    ...
    rows: [
      ...
      ["2", "任意合法分割", "—", "令 W=abcde，满足 |bcd|≤p、|bd|>0", "lemma 声称 ∀i≥0：abⁱcdⁱe∈L", "后续每类只需找一个反例 i"],
    ...
    steps: [
      ...
      "让对手任选 W=abcde。Pumping lemma 要求 |bcd|≤p、|bd|>0，并声称对所有 i≥0 都有 abⁱcdⁱe∈L。后面只需针对对手给出的任意合法分割，找出一个 i 使该串不在 L。",
    ...
    given: "语言由恰有一个分隔符的二进制串 left<right 构成，并要求 left 的数值严格小于 right。反设它是 CFL 后，任意合法分割 W=abcde 都必须满足：对所有 i≥0，abⁱcdⁱe 仍在语言中。",
