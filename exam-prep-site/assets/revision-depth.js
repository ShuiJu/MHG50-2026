...
steps: [
          ...
          },
          {
            ...
            steps: [
              ...
              "先推导 Garner 公式，而不是直接背减法顺序。目标是同时满足 x≡a_p (mod 523) 和 x≡a_q (mod 547)。令 x=a_p+523u，这样 x−a_p=523u，第一条同余式自动成立。再把它代入第二条：a_p+523u≡a_q (mod 547)。两边减去 a_p，得到 523u≡a_q−a_p (mod 547)。两边乘 523⁻¹≡433 (mod 547)，得到 u≡(a_q−a_p)×433 (mod 547)。因此差值的顺序由 x=a_p+523u 决定，必须是 a_q−a_p。",
              "根 (a_p=415, a_q=62)：u≡(62−415)×433≡(−353)×433 (mod 547)。先把负数归一：−353 mod 547=194。然后直接计算 u=194×433 mod 547=311。代回 x=a_p+523u：x=415+523×311=163068。",
              "根 (a_p=415, a_q=485)：a_q−a_p=485−415=70。直接计算 u=70×433 mod 547=225。代回 x=415+523×225=118090。",
              "根 (a_p=108, a_q=62)：公式要求先算 a_q−a_p，所以这里是 62−108，而不是 108−62。先把差值归一：−46 mod 547=501。然后直接计算 u=501×433 mod 547=321。再代回 x=a_p+523u：x=108+523×321=167991。检查减法方向：167991 mod 523=108，且 167991 mod 547=62，两条目标同余式都成立。若只把差值反成 108−62、却仍使用 x=a_p+523u，则会得到 u=226、x=118306，而 118306 mod 547=154≠62，所以那种写法不满足第二条条件。108−62 并非永远不能用：若把参数式同时改成 x=a_p−523u，那么会得到同一个解；不能只反转差值而保留其余公式不变。",
              "根 (a_p=108, a_q=485)：a_q−a_p=485−108=377。直接计算 u=377×433 mod 547=235。代回 x=108+523×235=123013。",
              "逐个验算候选根，直接计算 x² mod 286081。结果是：163068² mod 286081=229755；118090² mod 286081=229755；167991² mod 286081=229755；123013² mod 286081=229755。四个余数都等于 radicand 0x3817b=229755，所以四个候选全部通过回代。",
              ...
              "回到倍点斜率公式 λ≡(分子)×(分母)⁻¹ (mod 37)。步骤 21 已算出分子≡1 (mod 37)、分母=28，所以先写 λ≡1×28⁻¹ (mod 37)。步骤 22 得到 28⁻¹≡4 (mod 37)；把 28⁻¹ 替换为 4，得到 λ≡1×4≡4 (mod 37)。现在才把 λ=4 代入坐标公式：x_{2P}=λ²−2x_P=16−50=−34 mod 37=3；y_{2P}=λ(x_P−x_{2P})−y_P=4×(25−3)−14=74 mod 37=0。所以 2P=(3,0)。",
              "现在求 P 的阶。阶是使 nP=O 的最小正整数 n。本题不需要继续计算 3P；步骤 23 已经得到 2P=(3,0)，这个 y 坐标为 0 的结果足以确定阶。",
              "椭圆曲线上点 (x,y) 的负元是 −(x,y)=(x,−y mod p)。因此 −(2P)=−(3,0)=(3,−0 mod 37)=(3,0)。",
              "上一步说明 −(2P)=2P。任何点 Q 与它的负元 −Q 相加都等于无穷远点 O，所以 2P+(−2P)=O。把 −2P 替换为 2P，得到 2P+2P=O。",
              "按照标量乘法的定义，2P+2P=4P。因此上一行就是 4P=O；我们已经找到一个能使 nP=O 的正整数 n=4。",
              "还要确认 4 是最小的。P=(25,14) 是普通曲线点，不是 O，所以阶不是 1；2P=(3,0) 也不是 O，所以阶不是 2。",
              "阶也不可能是 3：如果 3P=O，同时又有 4P=O，那么 4P−3P=P 也会等于 O，这与 P=(25,14)≠O 矛盾。因此最小的正整数是 4，结论为 ord(P)=4。"
            ],
        ...
        parts: [
          ...
          },
          {
            ...
            steps: [
              ...
              "题目给 5P=(4,6)、P=(13,2)，所以 6P=5P+P。不同点加法的斜率式是 λ≡(y₂−y₁)×(x₂−x₁)⁻¹ (mod 17)。按同一顺序相减，分子=6−2=4，分母=4−13=−9≡8 (mod 17)，所以先写 λ≡4×8⁻¹ (mod 17)。用 Euclid 求逆：17=2×8+1，改写为 1=17−2×8，因此 8⁻¹≡−2≡15 (mod 17)。把 8⁻¹ 替换为 15：λ≡4×15 mod 17=9。再算 x≡9²−4−13 mod 17=13，y≡9×(4−13)−6 mod 17=15。因此 6P=(13,15)。",
              "题给 3Y=(4,11)，要验算 6Y=3Y+3Y。倍点斜率式是 λ≡(3x²+a)×(2y)⁻¹ (mod 17)。分子 3×4²+5=53，53 mod 17=2；分母 2×11=22，22 mod 17=5，所以先写 λ≡2×5⁻¹ (mod 17)。用 Euclid 求逆：17=3×5+2；5=2×2+1；反代得到 1=7×5−2×17，因此 5⁻¹≡7 (mod 17)。把 5⁻¹ 替换为 7：λ≡2×7 mod 17=14。再算 x_3=14²−2×4 mod 17=1，y_3=14×(4−1)−11 mod 17=14，所以 6Y=(1,14)。",
              "算 X = u1·P + u2·Y = 6P + 6Y = (13,15)+(1,14)。不同点加法：λ=(14−15)/(1−13) mod 17 = (−1)/(−12) mod 17。",
              "先归一分子和分母：−1 mod 17=16，−12 mod 17=5，所以斜率式变成 λ≡16×5⁻¹ (mod 17)。上一步已经算出 5⁻¹≡7 (mod 17)；把 5⁻¹ 替换为 7，得到 λ≡16×7 mod 17=10。",
              "把 λ=10 代入 x 坐标公式：x_X≡λ²−x₁−x₂≡10²−13−1≡86 mod 17=1。",
              "把 λ=10 和 x_X=1 代入 y 坐标公式：y_X≡λ(x₁−x_X)−y₁≡10×(13−1)−15≡105 mod 17=3。所以 X=(1,3)。",
...
(() => {
  ...
  exam[2].parts[2].states = [
    ...
    {after:5,title:"6P 的点加法状态",headers:["项目","计算","mod 17 结果"],rows:[["输入点","5P=(4,6)，P=(13,2)","—"],["斜率分子","6−2","4"],["斜率分母","4−13=−9","8"],["分母逆元","8⁻¹","15"],["斜率式","λ≡4×8⁻¹；代入 8⁻¹≡15","λ≡4×15≡9"],["x","9²−4−13","13"],["y","9(4−13)−6","15"],["结果","—","6P=(13,15)"]]},
    {after:6,title:"6Y 的倍点状态",headers:["项目","计算","mod 17 结果"],rows:[["输入点","3Y=(4,11)","—"],["分子","3×4²+5","2"],["分母","2×11","5"],["分母逆元","5⁻¹","7"],["斜率式","λ≡2×5⁻¹；代入 5⁻¹≡7","λ≡2×7≡14"],["x","14²−2×4","1"],["y","14(4−1)−11","14"],["结果","—","6Y=(1,14)"]]},
    {after:8,title:"X=6P+6Y 的斜率状态",headers:["分子","分母","分母逆元","代回斜率式","结果"],rows:[["14−15≡16 (mod 17)","1−13≡5 (mod 17)","5⁻¹≡7 (mod 17)","λ≡16×5⁻¹；代入后 λ≡16×7","λ≡10 (mod 17)"]]},
...
(() => {
  ...
  Object.assign(findExample("cs616", 0, "用 CRT 合并两个余数条件"), {
    ...
    steps: [
      ...
      "回到 CRT 组合公式 x≡a₁M₁y₁+a₂M₂y₂ (mod M)。这里 a₁=2、M₁=5、y₁=2，a₂=3、M₂=3、y₂=2，M=15。逐项代入得到 x≡2×5×2+3×3×2≡20+18≡38≡8 (mod 15)。",
  ...
  depth.cs616.learn[0].extraExamples.push({
    ...
    steps: [
      ...
      "回到 CRT 组合公式 x≡a₁M₁y₁+a₂M₂y₂+a₃M₃y₃ (mod M)。把前三行已经得到的 a_i、M_i、y_i 逐行代入：第一项 a₁M₁y₁=216×43621×14=131909904；第二项 a₂M₂y₂=50×56153×80=224612000；第三项 a₃M₃y₃=179×42173×120=905876040。因此 x≡131909904+224612000+905876040 (mod 10163693)。",
      "先求和 S=131909904+224612000+905876040=1262397944。考试中直接计算 S mod 10163693=2100012；这里需要的是余数 2100012，不需要另外记录整数商。",
    ...
    states: [
      ...
      {after:19,title:"求和与直接取模",headers:["计算","结果"],rows:[["第一项+第二项","131909904+224612000=356521904"],["再加第三项 S","356521904+905876040=1262397944"],["直接取模","1262397944 mod 10163693=2100012"]]},
